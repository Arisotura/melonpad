...
void LCD_SetBrightness(int brightness)
{
    ...
    LCD_JDI_SetBrightness(brightness);
    I2C_Finish();
...
int LCD_TryGetID()
{
    ...
    if (!I2C_Write(I2C_DEV_LCD, buf, 2, 0)) return 0;

    buf[0] = 0xBF;
    if (!I2C_Write(I2C_DEV_LCD, buf, 1, 1)) return 0;
    if (!I2C_Read(I2C_DEV_LCD, buf, 5)) return 0;
...
void LCD_JDI_Init()
{
    ...
    buf[0] = 0xB0; buf[1] = 0x02;
    I2C_Write(I2C_DEV_LCD, buf, 2, 0);

    buf[0] = 0xBB; buf[1] = 0x08; buf[2] = 0x7A; buf[3] = 0x01; buf[4] = 0x00;
    I2C_Write(I2C_DEV_LCD, buf, 5, 0);
...
void LCD_JDI_SetBrightness(int brightness)
{
    ...
    I2C_Write(I2C_DEV_LCD, buf, 6, 0);

    buf[0] = 0xB0; buf[1] = 0x03;
    I2C_Write(I2C_DEV_LCD, buf, 2, 0);

    buf[0] = on ? 0x29 : 0x28;
    I2C_Write(I2C_DEV_LCD, buf, 1, 0);
}
